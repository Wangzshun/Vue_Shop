/* eslint-disable no-useless-return */ /* eslint-disable camelcase */
<template>
  <div>
    <!-- 面包屑导航 -->
    ...
    <el-card>
      ...
      <el-row class="cat_opt">
        <el-col>
          <span>选择商品分类:</span>
          <!-- 选择商品分类的级联选择框 -->
          <el-cascader v-model="selectedCateKeys" :options="catelist" :props="{ expandTrigger: 'hover', value: cateProps.value, label: cateProps.label, children: cateProps.children }" @change="handleChange"></el-cascader>
        </el-col>
      </el-row>
      <!-- tab页签区域 -->
...
export default {
  ...
  methods: {
    ...
    async getParamsData() {
      ...
      // 通过判断this.activeName的值来决定渲染哪个table
    ...
    showInput(row) {
      ...
      // 自动获得焦点
      // $nextTick方法的作用就是当页面上的元素被重新渲染之后才会执行回调函数中的代码
      this.$nextTick((_) => {
